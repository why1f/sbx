...
## 8. TUI:「服务管理」栏与两行式服务器列表

### 8.1 token 管理

- **`[a]` 新增 agent**:填名称 → 主控生成 **32 字节随机 token**(`rand::rngs::OsRng`),base64url 编码,
  **只在弹窗里显示这一次**;库里只存 `token_hash` 与 `token_prefix`(前 8 位)。
  弹窗同时给出**可直接粘贴的 agent 侧一键安装命令**,含 `--server`、`--token`、`--fingerprint`。
