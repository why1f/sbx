...
- **`[a]` 新增 agent**:填名称、网卡月配额、重置日 → 主控生成 **32 字节随机 token**(`rand::rngs::OsRng`),
  ...
  **主控地址不问人**(`install::resolve_host`):配了订阅域名就用域名 —— 域名比 IP 好,
  换机器时被控端不用重配,TLS 那一套也才说得通;没配就探本机出口地址
  (`UdpSocket::connect` 到一个公网地址,只让内核选路由,**不发包**、不依赖外部服务、
  不会卡住)。代价是 NAT 后面的机器探到的是内网地址,那时去「设置」页填订阅对外地址覆盖。
  比去请求 ipify 之类的外部服务好:那会在无外网的内网部署里卡住十几秒,
  还给 TUI 引入一个网络依赖。
...
「主机」列只在**宽到有余量**时出现(约 140 列以上)。它是最锦上添花的一列,
不该从流量或重置日那里抢地方。
