...
// sing-box 版本就是这一行 require;升级 = go get -u(DESIGN.md §0.3 结论三、§11.1)。
// **不 fork、不打补丁。** 若这里出现 replace 指令或 patches/ 目录,
// 说明 §7.5 的触发条件被误判了。
//
// build tag 只允许两个,而且是**必需**的:`-tags with_quic,with_utls`
// (reality 依赖 uTLS,hysteria2 / tuic 依赖 QUIC)。
// 漏了会被 boxctl/buildtags.go 的编译期哨兵挡住。多了(尤其 with_v2ray_api)
// 说明 §0.2 被误读了 —— 流量统计走 ConnectionTracker,不需要 v2ray API。
//
// 版本已由 §12.0 的 spike 实测核对:v1.14.0-beta.3 是真实存在的 tag,
// 而原先占位写的 v1.14.0 并不存在(那时无 Go 工具链,无法验证)。
// sing / sing-tun 的版本由 sing-box 的 go.mod 传递决定,**不要手写**。
require (
