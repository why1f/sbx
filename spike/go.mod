...
// 刻意**不预先 pin 版本**。
//
// DESIGN.md §0.2 的源码核实基准是 sing-box v1.14.0-beta.3(commit 1569c529),
// 所以 spike 应当对着那个版本验:
//
//	go get github.com/sagernet/sing-box@v1.14.0-beta.3
//	go mod tidy
//
// sing 的版本由 sing-box 的 go.mod 传递决定,不要手写 —— 手写一个不匹配的
// sing 版本会让 bufio.NewInt64CounterConn 的签名对不上,而报错信息
// 看起来像是 §0.2 的机制不成立(实际上只是版本错配)。
//
// 若 v1.14.0-beta.3 已不可用,换成当时的最新 tag,并**在 DESIGN.md §0.2 里
// 更新核实基准那一行** —— 那行写的是「已逐条复核一致」,换了版本就得重新复核。
